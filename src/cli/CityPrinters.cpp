...
void printHelp() {
  std::cout << "UrbanSimCore CLI v0.1.0\n"
            ...
            << "  --ticks N                Run full autonomous RCI city for N ticks (default: 100;\n"
            << "                           same engine as --simulate when no other commands are set)\n"
            ...
            << "  --zone-rect X1 Y1 X2 Y2 TYPE  Zone a rectangle via ZoneTool (no treasury;\n"
            << "                               PARK/NONE are sandbox-only and skip player rules)\n"
            ...
            << "  --place-road X1 Y1 X2 Y2  Build a Manhattan (x-then-y) road via RoadTool (no treasury)\n"
            ...
            << "  --add-service TYPE X Y DIST  Place a service via ServiceTool (no treasury;\n"
            << "                               road access not required — coverage fixture)\n"
            << "  --add-power-source SOURCE X Y DIST MW  Place a typed Power facility via ServiceTool\n"
            << "                               (no treasury / road gate; then set SOURCE and MW)\n"
            ...
            << "  --print-city-summary      Print consolidated CityMetrics (uses live stock;\n"
            << "                           --seed-population optional)\n"
            ...
            << "  --simulate-disasters      Enable fire ignition/spread and earthquake/flood events during --simulate (off by default; destructive)\n"
